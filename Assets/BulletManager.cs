...
public class BulletManager : MonoBehaviour
{
    ...
    public void CreateBullet()
    {
        if (!shot)
        {
            ...
            bullet_instance.GetComponent<BoxCollider2D>().enabled = true;
    ...
    public void Update()
    {
        for (int c = 0; c < bulletList.Count; c++)
        {
            if (bulletList[c] != null)
            {
                ...
                if (OutofBounds(bulletList[c]))   //delete bullet when it move out of bounds or hits something
                {
    ...
    private void EraseBullet(GameObject target)
    {
        for (int c = 0; c < bulletList.Count; c++)
        {
            if (bulletList[c] == target)
            {
                bulletList[c].SetActive(false);
                bulletList.Remove(bulletList[c]);
                break;  
